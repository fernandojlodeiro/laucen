// Crea las tablas del Radar si no existen, corriendo db/radar.sql (que es
...
export function asegurarEsquema(): Promise<void> {
  listo ??= (async () => {
    const sql = await readFile(path.join(process.cwd(), "db", "radar.sql"), "utf8");
    const cliente = await pool.connect();
    try {
      await cliente.query("begin");
      await cliente.query("select pg_advisory_xact_lock(7212001)");
