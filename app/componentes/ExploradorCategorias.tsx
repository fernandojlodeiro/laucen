...
export default function ExploradorCategorias({ modo, irA, campo = "cats", inicio = RAIZ, iniciales = [], alto = "70vh", soloBuscador = false }: {
  ...
  const entrar = (id: string) => { setActual(id); setTexto(""); };
  const lista = resultados ?? hijas;

  function Fila({ c, conRuta }: { c: CatLigera; conRuta: boolean }) {
    const ya = elegidas.some((e) => e.id === c.id);
    return (
      <li className="flex items-center gap-2 px-3 py-1.5 border-b last:border-0 border-[#E3E9F0] text-xs">
        {modo === "ir" ? (
          <Link href={`${irA ?? ""}${c.id}`} className="flex-1 text-[#16577F] hover:underline">{conRuta ? relativa(c) : c.nombre}</Link>
        ) : !c.esHoja ? (
          <button type="button" onClick={() => entrar(c.id)} className="flex-1 text-left text-[#16577F] hover:underline" title="Entrar a esta categoría">
  ...
  return (
    <div className="grid gap-2 text-xs">
      ...
      {(!soloBuscador || resultados) && <div className="border border-[#E3E9F0] rounded-lg bg-white overflow-y-auto" style={{ maxHeight: alto, minHeight: "12rem" }}>
